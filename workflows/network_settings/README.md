...
3. ### How to run
    ...
    -> cmd to run: 
    ```bash
    ANSIBLE_PYTHON_INTERPRETER="$(which python)" ansible-playbook -i ./inventory/demo_lab/inventory_demo_lab.yml ./workflows/network_settings/playbook/network_settings_playbook.yml --tags t -vvv
    ```
    Structure file:
    ![alt text](./images/structure_file_1.png)

    #### b. Example of a playbook run with vars_files
    ```yaml
    tasks:
      ...
      ...
      - name: Create/Update Global Pools, Subpools and Network Settings etc 
        cisco.dnac.network_settings_workflow_manager:
          <<: *dnac_login
          state: merged
          config: "{{ network_servers }}"
        when: network_settings_details is defined
      ...
    ```
    -> cmd to run: 
    ```bash
    ANSIBLE_PYTHON_INTERPRETER="$(which python)" ansible-playbook -i ./inventory/demo_lab/inventory_demo_lab.yml ./workflows/network_settings/playbook/network_settings_playbook.yml --extra-vars VARS_FILE_PATH=./../vars/network_settings_vars.yml -vvv 
    ```
    Structure file:
    ![alt text](./images/structure_file_2.png)
    ...
    ```
    ![alt text](./images/inventory_demo_lab.png)
    ![alt text](./images/network_settings_playbook.png)
    ![alt text](./images/network_settings_vars.png)
