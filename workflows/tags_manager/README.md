...
ansible-playbook -i ./inventory/demo_lab/hosts.yaml ./workflows/tags_manager/playbook/tags_manager_playbook.yml -vvvv
```
...
```bash
  ...
  -v workflows/tags_manager/vars/tags_manager_inputs.yml
```

Return result validate:
```code
  (pyats-ansible-phamdat) bash-4.4$ ./tools/schemavalidation.sh -s workflows/tags_manager/schema/tags_manager_schema.yml -v workflows/tags_manager/vars/tags_manager_inputs.yml
...
2. Copy the sample workflow data from `workflows/tags_manager/vars/tags_manager_inputs.yml` into that inventory vars file.
3. Keep the same top-level variable name in inventory: `tags_details`.
4. Run the playbook without `VARS_FILE_PATH`:

```bash
ansible-playbook -i <inventory-file> workflows/tags_manager/playbook/tags_manager_playbook.yml -vvvv
```
...
- Relative to the playbook: `../vars/tags_manager_inputs.yml`
- Fully resolved from the repo root: `${PWD}/workflows/tags_manager/vars/tags_manager_inputs.yml`
